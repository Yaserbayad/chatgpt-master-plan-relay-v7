...
for(const x of ['composer-submit-button-color','Start Voice','Send prompt','LIGHT_COPY_SENTINEL_','copy-back did not replace sentinel','normalizeEditorClipboard','\\u00A0+$','baseline_submit_aria','primer_submit_aria','primer_cleared_aria','pasted_submit_aria','copy_sentinel_replaced','staged_copy_exact','bridge_prompt_sha256','SEND_AMBIGUOUS','SEND_AMBIGUOUS_NO_RETRY','LIGHT_MAX_SENDS = 1',"const INPUT_PRIME = 'x';",'INPUT_PRIME_FAILED','INPUT_PRIME_CLEAR_FAILED','${KEY_CTRL+KEY_V}','${KEY_CTRL+KEY_A}','${KEY_BACKSPACE}','${KEY_CTRL+KEY_C}'])assert.ok(watcher.includes(x),`watcher missing ${x}`);
assert.ok(!watcher.includes('firstEnabled('),'must not use failed selector-only Send discovery helper');
assert.ok(!watcher.includes('all(l,timeout,true).filter(enabled)'),'must not click a match obtained only via includeHidden:true');
assert.equal((watcher.match(/uiv\.browser\.type/g)||[]).length,6);
assert.ok(watcher.includes("const COMPOSER = 'css=[role=\"textbox\"][contenteditable=\"true\"][aria-label=\"Chat with ChatGPT\"]';"),'watcher must use Q08 composer locator');
assert.equal((watcher.match(/uiv\.browser\.click\(COMPOSER\)/g)||[]).length,4,'primer, clear, paste, copy-back must refocus exact composer');
assert.equal((watcher.match(/uiv\.browser\.type\(INPUT_PRIME\)/g)||[]).length,1,'exactly one constant ordinary trusted primer');
...
assert.ok(!runner.includes('@($Chrome,$Launcher,$MacroDir,$WatcherSource,$BridgeSource,$SchemaSource)'),'macro staging directory must self-heal');
